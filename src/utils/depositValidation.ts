...
export async function validateDepositData(
	...
	const validDeposits: DepositDataJson[] = deposits.filter(
	...
	if (validDeposits.length !== deposits.length) {
		throw new Error('Some of the deposits have already been made to the validators in this file.');
	}
	...
	// Validate balance
	const totalDepositAmount = validDeposits.reduce(
		(acc, deposit) => acc + parseGwei(deposit.amount.toString()) / 32n,
		0n,
	);
