...
async function fetchPubkeysByCredential(withdrawal_address: string, chainId: number): Promise<IndexerRecord[]> {
  console.log(chainId === 10200 ? CHIADO_VALIDATORS_API_URL : GNOSIS_VALIDATORS_API_URL);
  const res = await fetch(chainId === 10200 ? CHIADO_VALIDATORS_API_URL : GNOSIS_VALIDATORS_API_URL, {
    method: 'POST',
    headers: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (chainId !== '10200' && chainId !== '100') return NextResponse.json({ error: 'Unsupported chainId' }, { status: 400 });
